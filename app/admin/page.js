          ...
          <div style={{background:"var(--card)",borderRadius:16,border:"1px solid var(--border)",padding:24}}>
            ...
            <div style={{marginTop:24,paddingTop:20,borderTop:"1px solid var(--border)"}}>
              <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",marginBottom:12}}>
                <input type="checkbox" checked={!!form.sendTelegram} onChange={e=>{const ck=e.target.checked;setForm(p=>{var fire=String.fromCodePoint(0x1F525);var plane=String.fromCodePoint(0x2708,0xFE0F);var pkg=String.fromCodePoint(0x1F4E6);var bang=String.fromCodePoint(0x203C,0xFE0F);var star=String.fromCodePoint(0x2B50,0xFE0F);var url=(typeof window!=="undefined"?window.location.origin:"");var nm=p.name||"Product";var sk=p.sku||"";var pr=p.salePrice||p.price||"0";var sr=parseInt(p.smellRating)||0;var hdr=editing==="new"?fire+" NEW DROP "+fire:fire+" Product Updated "+fire;var lines=[hdr,"<b>"+nm+"</b>","SKU: "+sk];if(sr>0){lines.push("Nose: "+star.repeat(sr));}lines.push("<b>$"+pr+"</b>"+plane);if(p.noDiscount){lines.push("No Additional Discounts"+bang);}lines.push("Shop now: "+url);var msg=lines.join("\n");return{...p,sendTelegram:ck,telegramMessage:ck?(p.telegramMessage||msg):p.telegramMessage};});}} style={{accentColor:"var(--accent)",width:18,height:18}}/>
