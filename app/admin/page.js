...
export default function AdminPanel() {
  ...
  const getNextSku = () => {
    const nums = products.map(p => { const m = (p.sku||"").match(/^PP(\d+)$/); return m ? parseInt(m[1]) : 0; });
    const max = nums.length > 0 ? Math.max(...nums) : 0;
    return "PP" + String(max + 1).padStart(5, "0");
  };
    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{background:"var(--card)",borderRadius:20,border:"1px solid var(--border)",padding:40,maxWidth:400,width:"100%"}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{width:48,height:48,borderRadius:12,background:"linear-gradient(135deg,#6b6b6b,#4a4a4a)",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:24,fontWeight:800,color:"#fff",marginBottom:16}}>PP</div>
    ...
    <div style={{minHeight:"100vh"}}>
      <header style={{background:"rgba(10,10,15,.88)",backdropFilter:"blur(20px)",borderBottom:"1px solid var(--border)",padding:"0 24px",position:"sticky",top:0,zIndex:100}}>
        <div style={{maxWidth:1100,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:64}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#6b6b6b,#4a4a4a)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:800,color:"#fff"}}>PP</div><span style={{fontFamily:"'Outfit'",fontWeight:700,fontSize:18}}>Admin Panel</span></div>
          ...
          <div style={{background:"var(--card)",borderRadius:16,border:"1px solid var(--border)",padding:24}}>
            ...
            <div style={{marginTop:24,paddingTop:20,borderTop:"1px solid var(--border)"}}>
              <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",marginBottom:12}}>
                <input type="checkbox" checked={!!form.sendTelegram} onChange={e=>{const ck=e.target.checked;setForm(p=>{var fire=String.fromCodePoint(0x1F525);var plane=String.fromCodePoint(0x2708,0xFE0F);var pkg=String.fromCodePoint(0x1F4E6);var bang=String.fromCodePoint(0x203C,0xFE0F);var star=String.fromCodePoint(0x2B50,0xFE0F);var point=String.fromCodePoint(0x1F449,0x1F3FC);var url=(typeof window!=="undefined"?window.location.origin:"");var nm=p.name||"Product";var sk=p.sku||"";var pr=p.salePrice||p.price||"0";var sr=parseInt(p.smellRating)||0;var hdr=editing==="new"?fire+" NEW DROP "+fire:fire+" Product Updated "+fire;var lines=[hdr,"<b>"+nm+"</b>","SKU: "+sk];if(sr>0){lines.push("Nose: "+star.repeat(sr));}lines.push("<b>$"+pr+"</b>"+plane);if(p.noDiscount){lines.push("No Additional Discounts"+bang);}lines.push(point+" "+url);var msg=lines.join("\n");return{...p,sendTelegram:ck,telegramMessage:ck?(p.telegramMessage||msg):p.telegramMessage};});}} style={{accentColor:"var(--accent)",width:18,height:18}}/>
