      ...
      <header style={{position:"sticky",top:0,zIndex:100,background:"rgba(10,10,15,.88)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:"1px solid var(--border)",padding:"0 24px"}}>
        <div style={{maxWidth:1280,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:64}}>
          <div style={{display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={()=>{setPage("shop");setSel(null);}}>
            <div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#6b6b6b,#4a4a4a)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:800,color:"#fff"}}>PP</div>
            <span style={{fontFamily:"'Outfit',sans-serif",fontWeight:700,fontSize:20}}>Pounds<span style={{color:"var(--accent)"}}>Plug</span></span>
          </div>
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:"var(--card)",borderRadius:16,border:"1px solid var(--border)",width:"100%",maxWidth:360,maxHeight:"85vh",display:"flex",flexDirection:"column",overflow:"hidden"}}>
            ...
            <div style={{padding:14,overflowY:"auto",flex:1}}>
              ...
              <div style={{display:"flex",gap:10}}>
                {sel.inStock!==false?<button onClick={()=>{addToCart(sel);setSel(null);}} style={{flex:1,padding:"11px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#6b6b6b,#4a4a4a)",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer"}}>Add to Cart</button>:<div style={{flex:1,padding:"11px",borderRadius:10,background:"var(--border)",color:"var(--dim)",fontSize:14,fontWeight:700,textAlign:"center"}}>Sold Out</div>}
            ...
            {filtered.map((product)=>{const inS=product.inStock!==false;return(<div key={product.id} style={{background:"var(--card)",borderRadius:16,border:"1px solid var(--border)",overflow:"hidden",cursor:"pointer",opacity:inS?1:0.55}} onClick={()=>setSel(product)}>
              ...
              <div style={{padding:"14px 16px 16px"}}><p style={{fontSize:10,color:"var(--dim)",fontWeight:600,letterSpacing:".06em",marginBottom:3}}>SKU: {product.sku}</p><h3 style={{fontSize:14,fontWeight:600,marginBottom:4,lineHeight:1.3,minHeight:36}}>{product.name}</h3>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  ...
                  <button onClick={e=>{e.stopPropagation();inS&&addToCart(product);}} style={{padding:"8px 16px",borderRadius:10,border:"none",background:inS?"linear-gradient(135deg,#6b6b6b,#4a4a4a)":"var(--border)",color:"#fff",fontSize:12,fontWeight:600,cursor:inS?"pointer":"default"}}>{!inS?"Sold Out":addedId===product.id?"Added!":"Add to Cart"}</button>
                </div>
              </div>
            </div>);})}
          </div>
        ...
        {page==="cart"&&<div style={{maxWidth:800,margin:"0 auto"}}>
          ...
          :<>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              <button onClick={copyOrd} style={{width:"100%",padding:"16px",borderRadius:14,border:"none",background:copied?"linear-gradient(135deg,var(--green),#16a34a)":"linear-gradient(135deg,#6b6b6b,#4a4a4a)",color:"#fff",fontSize:16,fontWeight:700,cursor:"pointer",transition:"all .3s"}}>{copied?"Order Copied!":"Copy Order Details"}</button>
